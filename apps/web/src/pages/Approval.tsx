...
export function Approval() {
  ...
  async function prepare() { setBusy(true); setError(""); try { const result = await api<{ checkoutUrl?: string; decision: { message: string } }>(`/api/v1/intents/${id}/checkout`, { method: "POST" }); if (result.checkoutUrl) setCheckoutUrl(result.checkoutUrl); else setError(result.decision.message); } catch (cause) { setError(cause instanceof Error ? cause.message : "Checkout preparation failed"); } finally { setBusy(false); } }
  ...
  if (!data) return <Centered><div className="route-glow" /><div className="panel relative p-8"><Badge tone="bad">Approval unavailable</Badge><div className="mt-4"><ErrorNotice message={error || "IntentLock could not be loaded."} /></div></div></Centered>;
  const confirmed = Boolean(data.intent.confirmedAt);
  return <Centered><div className="route-glow" /><Reveal className="relative"><div className="mb-7 text-center"><span className="mx-auto grid h-16 w-16 place-items-center rounded-2xl border border-mint/25 bg-mint/[.09] text-mint"><LockKeyhole size={27} /></span><p className="eyebrow mt-6">Buyer-bound authorization chamber</p><h1 className="mt-3 text-4xl font-semibold tracking-tight">Review your IntentLock</h1><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-white/45">The link has no approval power by itself. You must be signed in as the same buyer and verify your enrolled passkey.</p></div>
    <div className="trust-preview"><div className="flex items-center justify-between gap-4 border-b border-white/[.08] bg-gradient-to-r from-mint/[.075] to-transparent p-5"><div><p className="font-mono text-[9px] uppercase tracking-wider text-white/35">Merchant-managed authoritative catalog</p><h2 className="mt-2 text-xl font-semibold">{data.product.name}</h2></div><Badge tone={confirmed ? "good" : "warn"}>{confirmed ? "Passkey approved" : "Awaiting you"}</Badge></div>
