...
export async function runMigrations(pool: Pool): Promise<void> {
  ...
  try {
    ...
    await client.query("SELECT pg_advisory_xact_lock(hashtext('spendseal:migrations'))");
    await client.query("CREATE TABLE IF NOT EXISTS agentrail_migrations (name text PRIMARY KEY, applied_at timestamptz NOT NULL DEFAULT now())");
    for (const name of files) {
      const applied = await client.query("SELECT 1 FROM agentrail_migrations WHERE name = $1", [name]);
