...
export async function handleMcpRequest(service: SpendSealService, store: SpendSealStore, browserAgent: BrowserAgentService, principal: OAuthPrincipal, req: Request, res: Response): Promise<void> {
  const server = new McpServer({ name: "spendseal", version: "3.2.0" }, { instructions: "SpendSeal is a bounded AI dealmaker. It lets an authenticated buyer agent make at most three increasing offers against a merchant's encrypted private authority, without revealing the merchant minimum. An accepted deal becomes one expiring PurchasePermit executed through Razorpay Test Mode. ChatGPT may negotiate and create the permit, but it cannot change the buyer ceiling, approve the deal, bypass the buyer's passkey, or invoke payment. Internal deal-session IDs must never be requested from the buyer: when an ID is unavailable, continue or retrieve the active deal by productId." });
  ...
  server.registerTool("start_price_negotiation", { title: "Start a bounded price negotiation", description: "Submit the authenticated buyer's first offer for one negotiation-enabled product. The private merchant minimum is never returned. At most three increasing buyer offers are allowed.", inputSchema: { productId: z.string().uuid(), buyerMaxTotalPaise: z.number().int().positive(), initialOfferPaise: z.number().int().positive(), idempotencyKey: z.string().min(8).max(120).optional() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true } }, async (input) => {
    ...
    const resumed = deal.roundCount > 1 || deal.rounds[0]?.buyerOfferPaise !== input.initialOfferPaise || deal.buyerMaxTotalPaise !== input.buyerMaxTotalPaise;
    const last = deal.rounds.at(-1); const state = last?.response === "counter" ? `The merchant agent countered at INR ${(last.merchantCounterPaise! / 100).toFixed(2)}. You may submit a higher offer that stays within the buyer's hard maximum.` : last?.response === "accepted" ? `Deal accepted at INR ${(deal.acceptedPricePaise! / 100).toFixed(2)}. Create the negotiated PurchasePermit for buyer approval.` : "NO_DEAL. No merchant floor was disclosed and no PurchasePermit or payment order was created.";
    const message = `${resumed ? `Resumed the existing active deal. Its original hard maximum remains INR ${(deal.buyerMaxTotalPaise / 100).toFixed(2)}, with ${3 - deal.roundCount} buyer offer(s) remaining. ` : ""}Deal session ${deal.id}. ${state}`;
    return { structuredContent: { deal }, content: [{ type: "text", text: message }] };
  });

  server.registerTool("counter_price_negotiation", { title: "Submit the next buyer offer", description: "Submit one strictly higher buyer offer in an active deal. Use productId when the internal deal-session ID is unavailable; never ask the buyer to provide an internal ID. The offer cannot exceed the buyer's original hard maximum.", inputSchema: { dealSessionId: z.string().uuid().optional(), productId: z.string().uuid().optional(), offerPaise: z.number().int().positive() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true } }, async ({ dealSessionId, productId, offerPaise }) => {
    requireScope(principal, "deals:create");
    if (!dealSessionId && !productId) return { isError: true, content: [{ type: "text", text: "Provide productId to continue the authenticated buyer's active deal. Do not ask the buyer for an internal deal-session ID." }] };
    const deal = dealSessionId ? await service.deals.counter({ buyerId: principal.userId, dealSessionId, offerPaise }) : await service.deals.counterActive({ buyerId: principal.userId, productId: productId!, offerPaise }); const last = deal.rounds.at(-1);
    const message = last?.response === "counter" ? `Deal session ${deal.id}. Merchant counter: INR ${(last.merchantCounterPaise! / 100).toFixed(2)}.` : last?.response === "accepted" ? `Deal session ${deal.id}. Deal accepted at INR ${(deal.acceptedPricePaise! / 100).toFixed(2)}.` : `Deal session ${deal.id}. NO_DEAL. No final counter is returned, and no PurchasePermit or Razorpay order exists.`;
    return { structuredContent: { deal }, content: [{ type: "text", text: message }] };
  });

  server.registerTool("get_price_negotiation", { title: "Get price negotiation", description: "Read the authenticated buyer's visible offers, merchant counters, status and accepted terms. Use productId when the internal deal-session ID is unavailable; never ask the buyer for it. The private merchant minimum is never exposed.", inputSchema: { dealSessionId: z.string().uuid().optional(), productId: z.string().uuid().optional() }, annotations: { readOnlyHint: true, openWorldHint: false, destructiveHint: false } }, async ({ dealSessionId, productId }) => {
    requireScope(principal, "deals:read");
    if (!dealSessionId && !productId) return { isError: true, content: [{ type: "text", text: "Provide productId to retrieve the authenticated buyer's active deal. Do not ask the buyer for an internal deal-session ID." }] };
    const deal = dealSessionId ? await service.deals.get(principal.userId, dealSessionId) : await service.deals.getActive(principal.userId, productId!);
    return { structuredContent: { deal }, content: [{ type: "text", text: `Deal session ${deal.id}. Status: ${deal.status}. Buyer offers used: ${deal.roundCount} of 3.` }] };
  });

  server.registerTool("create_negotiated_purchase_permit", { title: "Seal an accepted deal", description: "Turn one accepted, unused deal into a single-use PurchasePermit. Use productId when the internal deal-session ID is unavailable; never ask the buyer for it. This does not approve or pay; the buyer must use a passkey.", inputSchema: { dealSessionId: z.string().uuid().optional(), productId: z.string().uuid().optional() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true } }, async ({ dealSessionId, productId }) => {
    requireScope(principal, "intents:create");
    if (!dealSessionId && !productId) return { isError: true, content: [{ type: "text", text: "Provide productId to seal the authenticated buyer's accepted deal. Do not ask the buyer for an internal deal-session ID." }] };
    const created = dealSessionId ? await service.deals.createPermit({ buyerId: principal.userId, dealSessionId }) : await service.deals.createPermitForProduct({ buyerId: principal.userId, productId: productId! }); const approvalUrl = created.approvalToken ? `${service.config.publicBaseUrl}/approve/${created.intent.id}?token=${encodeURIComponent(created.approvalToken)}` : null;
