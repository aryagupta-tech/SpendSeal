...
try {
  await runMigrations(pool); const store = new AgentRailStore(pool); const user = await store.getUserByUsername(username);
  if (!user) throw new Error(`No AgentRail user exists for ${username}; register a passkey first`);
  const result = await seedNovaDesk(store, new AgentRailService(store, config), user.id);
