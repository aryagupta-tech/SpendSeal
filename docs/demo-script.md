...
Show the AgentRail dashboard and the pipeline card.

## 0:35–1:40 — Successful agentic purchase

In ChatGPT, with the AgentRail MCP connection enabled, send:
...
Let ChatGPT list the catalog and create the IntentLock. Point out the structured constraints and open the approval URL. Say: “There is no MCP approval tool. The model cannot grant itself authority, and possession of this link alone is not approval.”
...
Create a second equivalent IntentLock and approve it. Before preparing checkout, use **Manipulate price** on the merchant dashboard. NovaDesk's authoritative server price changes from ₹999 to ₹1,299.

Ask ChatGPT to continue. AgentRail returns `PRICE_CHANGED` and `BUDGET_EXCEEDED`; no Razorpay order is created.

## 2:45–3:30 — Duplicate prevention

Ask ChatGPT to prepare checkout again for the already-paid first IntentLock. Show `REPLAY_DETECTED` and the unchanged provider-order count.
...
- IntentLock records exact permission.
...
> ChatGPT interprets. A passkey approves. AgentRail enforces. Razorpay moves the money.
